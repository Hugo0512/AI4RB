...
datagenerator1 = DataGenerator(data_dir='/home/zhl/copydata',patient_list_txtfile='train5.txt',batch_size=24,shuffle=True)
...
# now=next(datagenerator1)
# print(now[0][0].shape,now[0][0].shape)
datagenerator2 = DataGenerator(data_dir='/home/zhl/copydata',patient_list_txtfile='test5.txt',batch_size=16,shuffle=True)
...
def check_print():
    # base_model=InceptionResNetV2(include_top=False,weights=None,input_shape=(187, 187, 55),backend=keras.backend,layers=keras.layers,models=keras.models,utils=keras.utils)
    base_model = ResNet101(include_top=False,weights='imagenet',input_shape=(256, 256, 3),backend=keras.backend,layers=keras.layers,models=keras.models,utils=keras.utils)
...
# class_weight={0:10.0,1:0.01} # for the first binary classification problem
...
trained=model.fit_generator(generator=datagenerator1,validation_data=datagenerator2,epochs=10,callbacks=[ModelCheckpoint(filepath='/home/zhl/resnet_5.h5',monitor='val_acc', verbose=1, save_best_only=True, mode='max')])
